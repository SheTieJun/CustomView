...
import com.tencent.liteav.demo.superplayer.timer.TimeType
...
class VideoFullTimeTypeListAdapter(data: ArrayList<TimeType>) : BaseQuickAdapter<TimeType, BaseViewHolder>(R.layout.superplayer_item_full_time_type, data) {
    ...
    override fun convert(holder: BaseViewHolder, item: TimeType) {
        ...
        holder.setTextColor(R.id.name, if (position == holder.bindingAdapterPosition - headerLayoutCount) getColor(context,R.color.orange) else getColor(context,R.color.white))
        holder.getView<TextView>(R.id.name).apply {
            typeface = if (position == holder.bindingAdapterPosition - headerLayoutCount) {
